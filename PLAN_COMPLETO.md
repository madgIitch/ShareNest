Con toda la info que necesito, voy a construir el plan completo.Aquí va el plan completo. Lo divido en una vista general primero, luego el detalle por sprints.Ahora el detalle técnico completo de cada sprint:

---

## Arquitectura base

Antes de empezar los sprints, la stack queda así:

**Frontend:** React Native + Expo SDK (managed workflow), Expo Router v3 para navegación, Zustand para estado global, TanStack Query para server state, NativeWind para estilos.

**Backend:** Supabase con PostgreSQL, Row Level Security en todas las tablas, Realtime para el chat, Storage para imágenes, Edge Functions para lógica serverless (templates, notificaciones), Auth con magic link + phone OTP.
...
## Dependencias clave (npm/Expo)

Las librerías más importantes que necesitarás instalar a lo largo de los sprints: `@supabase/supabase-js`, `@tanstack/react-query`, `zustand`, `nativewind`, `expo-router`, `react-native-maps`, `@gorhom/bottom-sheet`, `react-native-reanimated`, `expo-image-picker`, `expo-notifications`, `expo-location`, `expo-secure-store`, `react-native-purchases` (RevenueCat), `maestro` para E2E.

---

## Resumen de timings

| Fase | Sprints | Semanas | Resultado |
|------|---------|---------|-----------|
| Base | 1–3 | 1–6 | Auth + perfil + UI system |
| Core | 4–6 | 7–12 | Producto funcional completo |
| Social | 7–8 | 13–16 | Diferenciadores de red social |
| Launch | 9 | 17–18 | Stores + monetización |

Con un equipo de 2 developers el timeline es realista. Con uno solo, añade un 30–40% de margen en los sprints 4–6 que son los más densos.
